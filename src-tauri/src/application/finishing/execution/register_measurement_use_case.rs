...
use crate::application::finishing::dto::{
    FinishingExecutionOutput, RegisterFinishingMeasurementInput,
};

use crate::domain::{machining::finishing::FinishingExecution, units::Diameter};
...
impl RegisterFinishingMeasurementUseCase {
    ...
    pub fn execute(
        ...
        let measurement = p.value("measurement_mm", Diameter::mm(input.measurement_mm));
        ...
        if let Some(measurement) = measurement {
            p.domain(
                "measurement_mm",
                execution.register_measurement(input.step_number, measurement),
            );
        }
