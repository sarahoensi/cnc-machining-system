...
impl FinishingPlanner {
    ...
    pub fn validate_direction(
        ...
        match mode {
            FinishingMode::Inner if t <= s => Err(FinishingError::InvalidModeDirection {
                start_mm: s,
                target_mm: t,
            }),

            FinishingMode::Outer if t >= s => Err(FinishingError::InvalidModeDirection {
                start_mm: s,
                target_mm: t,
            }),
    ...
    pub fn validate_radial_engagement(ae: PositiveLength) -> Result<(), FinishingError> {
        if ae.mm_value() <= 0.0 {
